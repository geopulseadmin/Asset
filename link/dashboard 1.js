var main_url = "https://portal.geopulsea.com/geoserver/"
...
$('#daterange').on('apply.daterangepicker', function (ev, picker) {
  ...
  function loadinitialData(cql_filter) {
    const filternames = ["DP", "Status", "Type", "Width"]; //accordn column names , if want add one more filter criteria add here

    filternames.forEach(function (filtername) {
      var url = `${main_url}MissingLinks/wms?service=WFS&version=1.1.0&request=GetFeature&typeName=Missing_Links_buffer_&propertyName=${filtername}&outputFormat=application/json&cql_filter=${encodeURIComponent(cql_filter)}`;
      console.log(url);
...
function DataTableFilter(cql_filter1) {
  var layers = ["MissingLinks:Missing_Links_buffer_"];
  var typeName = layers.join(',');
  var cqlFilter = cql_filter1;
  var geoServerURL =
    `${main_url}MissingLinks/wms?service=WFS&version=1.1.0&request=GetFeature&typeName=${typeName}&outputFormat=application/json&CQL_FILTER=${encodeURIComponent(cqlFilter)}`;
  var headers = ['DP', 'Status', 'Type', 'Width'];
...
function getCheckedValues(callback) {
  var selectedValues = {};
  const filternames = ["DP", "Status", "Type", "Width"];
...
// for search button
  ...
  document.addEventListener('DOMContentLoaded', (event) => {
    var columns = {"DP":"DP", "Status":"Status", "Type":"Type", "Width":"Width"};
  ...
  $("#search_type").change(function () {
    ...
    function getValues(callback) {
      var geoServerURL = `${main_url}MissingLinks/wms?service=WFS&version=1.1.0&request=GetFeature&typeName=Missing_Links_buffer_&propertyName=${selectedValue}&outputFormat=application/json`;
      ...
      for (i = 0; i < arr.length; i++) {
        if (arr[i].toLowerCase().includes(val)) { // Check if the suggestion contains the input value
          ...
          item.addEventListener("click", function () {
            ...
            console.log("Adding Missing_Links_buffer layers with filter:", cqlFilter);
...
const layerDetails = {
  "pmc:Missing_Links_buffer_": ['DP', 'Status', 'Type', 'Width'],
